...
TEST(RastLineFinderTestSuite, test1) {
  ...
  // line 1
  ASSERT_TRUE(!finder.findNext(&supportIdxs).isNull());
  ASSERT_TRUE(matchSupportPoints(supportIdxs, line1Idxs));

  // line2
  ASSERT_TRUE(!finder.findNext(&supportIdxs).isNull());
  ASSERT_TRUE(matchSupportPoints(supportIdxs, line2Idxs));
