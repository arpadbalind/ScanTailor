...
TEST(MatrixCalcSuite, test1) {
  ...
  for (int i = 0; i < 3; ++i) {
    ExpectClose(x[i], control[i], 1e-6);
  }
}
...
TEST(MatrixCalcSuite, test2) {
  ...
  B[4] += 1.0;
  EXPECT_THROW(mc(A, 3, 5).trans().solve(mc(B, 5, 1)), std::runtime_error);
}
...
TEST(MatrixCalcSuite, test3) {
  ...
  for (int i = 0; i < 9; ++i) {
    ExpectClose(inv[i], control[i], 1e-6);
  }
}
...
TEST(MatrixCalcSuite, test4) {
  ...
  for (int i = 0; i < 8; ++i) {
    ExpectClose(mul[i], control[i], 1e-6);
  }
}
