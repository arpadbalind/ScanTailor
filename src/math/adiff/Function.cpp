...
Function<2>::Function(std::size_t numNonZeroVars) : value(), firstDerivs(numNonZeroVars), secondDerivs(numNonZeroVars) {}
...
Function<2>::Function(std::size_t argIdx, double val, const SparseMap<2>& sparseMap)
  ...
  const std::size_t numVars = sparseMap.numVars();

  // argIdx row
  for (std::size_t i = 0; i < numVars; ++i) {
    const std::size_t u = sparseMap.nonZeroElementIdx(argIdx, i);
  ...
  // argIdx column
  for (std::size_t i = 0; i < numVars; ++i) {
    const std::size_t u = sparseMap.nonZeroElementIdx(i, argIdx);
    if (u != SparseMap<2>::ZERO_ELEMENT) {
      firstDerivs[u] = 1.0;
    }
  }
...
VecT<double> Function<2>::gradient(const SparseMap<2>& sparseMap) const {
  ...
  for (std::size_t i = 0; i < numVars; ++i) {
    const std::size_t u = sparseMap.nonZeroElementIdx(i, i);
    if (u != SparseMap<2>::ZERO_ELEMENT) {
...
MatT<double> Function<2>::hessian(const SparseMap<2>& sparseMap) const {
  ...
  for (std::size_t i = 0; i < numVars; ++i) {
    for (std::size_t j = 0; j < numVars; ++j) {
      double Fij = 0;
      const std::size_t ij = sparseMap.nonZeroElementIdx(i, j);
...
Function<2>& Function<2>::operator+=(const Function<2>& other) {
  const std::size_t p = firstDerivs.size();
...
Function<2> operator+(const Function<2>& f1, const Function<2>& f2) {
  const std::size_t p = f1.firstDerivs.size();
...
Function<2> operator/(const Function<2>& num, const Function<2>& den) {
  const std::size_t p = num.firstDerivs.size();
